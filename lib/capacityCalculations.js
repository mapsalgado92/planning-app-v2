export const generateCapacity = (capPlan, entries, weeks, fromWeek) => {
  ...
  let newPlan = weeks.map((week) => {
    ...
    let newRecentItem = {
      ...
      fcAttrition: entry ? (parseFloat(entry.fcAttrition) || 0) * parseFloat(current.totalFTE) : 0
    }
    ...
    if (entry && entry.fcAttrition) {
      ...
      if (current.isFuture) { 
        newPlanWeek.expectedFTE =
          Math.round(
            newPlanWeek.expectedFTE * (1 - newPlanWeek.expectedAttrition / 100) * 100
          ) / 100
      }
    }
    ...
    if (current.pShrinkage) {
      ...
      newPlanWeek.pAbs = newShrink.aux
      newPlanWeek.pAux = newShrink.abs
